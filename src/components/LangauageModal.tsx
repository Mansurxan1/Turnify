...
const LanguageModal = () => {
  ...
  // Mobil klaviatura uchun dinamik balandlik
  useEffect(() => {
    const handleResize = () => {
  ...
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    let cleanedValue = value.replace(/^\s+/, "").replace(/\s+/g, " ");
    if (/^[a-zA-Z\s]*$/.test(cleanedValue)) {
      setName(cleanedValue);
    }
  };
  ...
  return (
    <>
      ...
      <div className={`flex flex-col items-center justify-center min-h-screen ${bgColor} px-8`}>
        <div className="w-full absolute pt-2 mx-auto max-w-[450px] top-10 flex justify-start">
          <div style={progressBarStyle}></div>
        </div>
        <h2 className={`text-3xl w-60 text-center font-inter font-bold ${textColor} mb-6`}>
      ...
      {showDetailsModal && (
        <div className="fixed inset-0 flex items-start justify-center z-50">
          <div
            className={`flex flex-col w-full max-w-[450px] min-h-screen ${bgColor}`}
            style={{ overflowY: "auto" }}
          >
            <div className="w-full absolute top-4 flex flex-col">
              <button
                onClick={() => (setShowDetailsModal(false), setProgress(33))}
                className={`${textColor} text-[24px] px-4 self-start`}
              >
            ...
            <div className="px-8 pt-24 pb-20 flex flex-col items-center w-full">
              <h2 className={`text-xl font-bold ${textColor} leading-tight text-center`}>
                {t("enter_details")}
              </h2>
              <p className={`mt-4 text-[#768C9E] text-sm leading-relaxed text-center`}>
                {t("name_instructions")}
              </p>
              <input
                ref={nameInputRef}
                value={name}
                onChange={handleNameChange}
                onFocus={() => nameInputRef.current?.scrollIntoView({ behavior: "smooth", block: "center" })}
                placeholder={t("your_name")}
                className={`w-full max-w-[390px] mt-4 p-3 ${buttonBg} rounded-xl text-[#5EB5F7] text-[16px] focus:outline-none`}
              />
              <p className={`mt-6 text-[#768C9E] text-sm leading-relaxed text-center`}>
                {t("username_instructions")}
              </p>
              <input
                ...
                value={username}
                onChange={handleUsernameChange}
                onFocus={() => usernameInputRef.current?.scrollIntoView({ behavior: "smooth", block: "center" })}
                placeholder="@username"
                className={`w-full max-w-[390px] mt-4 p-3 ${buttonBg} rounded-xl text-[#5EB5F7] text-[16px] focus:outline-none`}
              />
              {usernameError && (
      ...
      {showTimeZoneModal && (
        <div className="fixed inset-0 flex items-start justify-center z-50">
          ...
          >
            ...
            <div className="px-8 pt-24 pb-20 flex flex-col items-center w-full">
              <h2 className={`text-[24px] font-bold ${textColor} leading-tight text-center`}>
                {t("select_timezone")}
              </h2>
              <p className={`mt-4 text-[#768C9E] text-[14px] leading-relaxed text-center`}>
                {t("timezone_instructions")}
              </p>
              <div className="w-full max-w-[420px] mt-6 relative" ref={dropdownRef}>
                <div className="relative">
